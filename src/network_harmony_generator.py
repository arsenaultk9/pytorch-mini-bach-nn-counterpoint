...
class NetworkHarmonyGenerator:
    ...
    def generate_harmony(self, x_soprano: torch.Tensor):
        ...
        for slice_index in range(constants.SEQUENCE_LENGTH):
            soprano_note = self.get_note_number(x_soprano[0], slice_index, voices['soprano'])
            alto_note = self.get_note_number(y_alto, slice_index, voices['alto'])
            tenor_note = self.get_note_number(y_tenor, slice_index, voices['tenor'])
            bass_note = self.get_note_number(y_bass, slice_index, voices['bass'])
